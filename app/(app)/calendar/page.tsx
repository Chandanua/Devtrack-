...
export default function CalendarPage() {
  ...
  const fetchTasks = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from('tasks').select('*, project:projects(name)').not('due_date', 'is', null).neq('status', 'completed');
    ...
    <div className="p-4 md:p-6 lg:p-8">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            {calendarDays.map((d, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'flex min-h-[80px] flex-col rounded-lg border p-1.5 text-left transition-colors hover:bg-muted/50',
                    selectedDate === key && 'border-primary ring-1 ring-primary',
                    isToday && 'bg-primary/5',
                    isPast && dayTasks.length > 0 && 'bg-destructive/5'
                  )}
                >
        ...
        <Card className="p-5">
          ...
          {!selectedDate ? (
            <EmptyState icon={CalIcon} title="No day selected" description="Click a calendar day to see tasks due that day." className="border-0" />
          ) : selectedTasks.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">No tasks due this day.</p>
          ) : (
              ...
              {selectedTasks.map((t) => (
                <Link key={t.id} href={`/tasks/${t.id}`} className="flex items-center gap-2 rounded-lg border p-3 transition-colors hover:bg-muted/50">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {t.project && <p className="text-xs text-muted-foreground">{(t.project as unknown as { name: string }).name}</p>}
                  </div>
                </Link>
              ))}
            </div>
