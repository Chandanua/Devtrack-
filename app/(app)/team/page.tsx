...
import { ROLE_LABELS, AVAILABILITY_META, initials, avatarGradient } from '@/lib/constants';
...
export default function TeamPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [{ data: memberData }, { data: taskData }] = await Promise.all([
      ...
      supabase.from('tasks').select('*, assignees:task_assignees(user:profiles(*))').is('parent_task_id', null),
    ]);
  ...
  const filtered = useMemo(() => members.filter((m) => m.full_name.toLowerCase().includes(search.toLowerCase()) || m.email.toLowerCase().includes(search.toLowerCase()) || (ROLE_LABELS[m.role] ?? 'Unknown').toLowerCase().includes(search.toLowerCase())), [members, search]);
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
      ...
      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 6 }).map((_, i) => <Card key={i} className="h-48 animate-pulse bg-muted/40" />)}</div>
      ) : filtered.length === 0 ? (
        <EmptyState icon={Users} title="No team members found" description={search ? "Try a different search term." : "No team members have signed up yet."} />
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((member, i) => {
            const stats = memberStats.get(member.id) ?? { total: 0, completed: 0, overdue: 0 };
            const avail = AVAILABILITY_META[member.availability] ?? AVAILABILITY_META.offline;
            return (
                  ...
                  <div className="flex items-start gap-4">
                    <div className="relative">
                      <Avatar className="h-12 w-12">
                        ...
                        <AvatarFallback className={cn('bg-gradient-to-br font-semibold text-white', avatarGradient(member.id))}>{initials(member.full_name)}</AvatarFallback>
                      </Avatar>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-muted-foreground">{member.job_title ?? ROLE_LABELS[member.role]}</p>
                      <Badge variant="secondary" className="mt-1.5 text-[10px]">{ROLE_LABELS[member.role] ?? 'Unknown'}</Badge>
                    </div>
                  </div>
